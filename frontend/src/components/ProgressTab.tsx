...
export function ProgressTab({ downloadStatus, resetForm }: ProgressTabProps) {
  ...
  const getStatusDisplay = (status: string | undefined, phase?: string) => {
    switch (status) {
      case 'downloading':
        return phase === 'converting' ? 'Converting...' : 'Downloading...';
      case 'processing':
  ...
  const getStatusIcon = (status: string | undefined) => {
    switch (status) {
      case 'completed':
        return (
          <CheckCircle className="h-12 w-12 dark:text-success text-success" />
        );
      case 'error':
        return <XCircle className="h-12 w-12 text-destructive" />;
      case 'downloading':
        return (
          <Loader2 className="h-12 w-12 dark:text-primary text-secondary animate-spin" />
        );
      case 'processing':
        return (
          <Loader2 className="h-12 w-12 dark:text-primary text-secondary animate-pulse" />
        );
      default:
        return (
          <Clock className="h-12 w-12 dark:text-primary-foreground/70 text-secondary-foreground/70" />
        );
    }
  };
  ...
  const getFileTypeIcon = (filename?: string) => {
    ...
    if (isPlaylist) {
      return (
        <ListMusic className="h-5 w-5 dark:text-primary text-secondary-foreground" />
      );
    } else if (isAudio) {
      return (
        <Music className="h-5 w-5 dark:text-primary text-secondary-foreground" />
      );
    } else {
      return (
        <Video className="h-5 w-5 dark:text-primary text-secondary-foreground" />
      );
    }
  };
    ...
    switch (status) {
      case 'completed':
        return {
          ...
          badge:
            'dark:bg-success/10 bg-success/10 dark:text-success text-success',
        };
      case 'error':
        return {
      ...
      case 'downloading':
        return {
          ...
          badge:
            'dark:bg-primary/20 bg-secondary/20 dark:text-primary-foreground text-secondary-foreground',
        };
      case 'processing':
        return {
          icon: 'dark:bg-primary/10 bg-secondary/10',
      ...
      default:
        return {
          ...
          background: 'dark:bg-primary/5 bg-secondary/5',
          badge:
            'dark:bg-primary/10 bg-secondary/10 dark:text-primary-foreground/70 text-secondary-foreground/70',
        };
    }
  };
  ...
  const progress = Math.round(downloadStatus.progress);
    ...
    >
        ...
        <Card
          className={`${
            downloadStatus.status === 'completed'
              ? 'dark:border-success/30 border-success/30'
              : downloadStatus.status === 'error'
              ? 'border-destructive/30'
              : 'dark:border-primary/20 border-secondary/30'
          } shadow-xl overflow-hidden rounded-xl
        ...
        >
          <CardContent className="p-7">
                ...
                >
                  ...
                  <div className="w-full">
                    <div className="flex justify-between text-xs mb-1">
                      <span className="dark:text-primary-foreground/70 text-secondary-foreground/70">
                        {downloadStatus.progress.toFixed(1)}%
                      </span>
                      {downloadStatus.downloaded_size &&
                        downloadStatus.total_size && (
                          <span className="dark:text-primary-foreground/70 text-secondary-foreground/70">
                            {downloadStatus.downloaded_size} /{' '}
                            {downloadStatus.total_size}
                          </span>
                  ...
                  {downloadStatus.speed && (
                    <div className="flex items-center gap-1.5 px-3 py-1 rounded-full dark:bg-primary/10 bg-secondary/10">
                      ...
                      <span className="text-xs dark:text-primary-foreground/80 text-secondary-foreground/80">
                        {downloadStatus.speed}
                        {downloadStatus.eta
                          ? ` • ${downloadStatus.eta}s remaining`
                          : ''}
                      </span>
                    </div>
                  )}
            ...
            >
              ...
              <Button
                variant={
                  downloadStatus.status === 'completed' ? 'outline' : 'default'
                }
                className={`w-full sm:w-auto flex-1 relative z-20 rounded-lg h-11 py-2.5 font-medium ${
                  downloadStatus.status === 'completed'
      ...
      {downloadStatus.status === 'completed' && (
        ...
        >
          ...
          <div>
            <h4 className="font-medium dark:text-success text-success">
              Download Complete
            </h4>
            <p className="text-sm mt-1 dark:text-success/80 text-success/80">
              Your file has been downloaded successfully. You can find it in the
              downloads folder.
            </p>
          </div>
        </motion.div>
